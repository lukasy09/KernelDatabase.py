...
class UserInteraction:
    model_path = "./model/digits_spoken.h5"
    digit_map = {
        0: 'zero',
        1: 'one',
        2: 'two',
    ...
    def __init__(self):
        self.model = load_model("./model/digits_spoken.h5")
        self.file_path = ""
        self.test_sample = None
        self.predictions = None
        print("Simple Spoken-Digit recon\nTo exit the program click 'q' & enter ")

    def handle_input(self):
        self.file_path = input("Enter a path to your audio file:")

    def prepare_sample(self):
        if self.file_path == "q":
            self.isRunning = False
            sys.exit(0)
        self.test_sample = AudioDataGenerator().preprocessSingleFile(self.file_path)

    def predict(self):
        self.predictions = self.model.predict(self.test_sample)

    def deliver_verdict(self):
        list_ = self.predictions.tolist()
        list_ = list_[0]
        found_index = list_.index(max(list_))
        found_index = self.digit_map[found_index]
        print('I think i hear `{}` in the recording'.format(found_index))
...
while interaction.isRunning:
        ...
        interaction.prepare_sample()
        interaction.predict()
        interaction.deliver_verdict()
